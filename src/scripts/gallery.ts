...
async function setupGallery(gallery: HTMLElement, groupIndex: number) {
  ...
  galleries.set(
    galleryId,
    sources.map(({ src, alt, caption }) => ({ src, alt, caption })),
  )

  const items: LayoutItem[] = await Promise.all(sources.map(async (source) => ({ src: source.src, aspectRatio: await aspectRatioOf(source.img, source.src) })))
